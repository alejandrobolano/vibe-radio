...
import { ArrowLeft, Broadcast, CursorClick, Heart, Pause, Play, ShareNetwork, ThumbsUp, TrendDown, TrendUp, WarningCircle } from '@phosphor-icons/react'
...
import { getLos40TestSources, isDevelopmentHost } from '../utils/los40TestSources'
...
export function StationPage({ route, player, favorites, sleepTimer, navigate }: StationPageProps) {
  ...
  const stationVote = useStationVote(station)
  const los40TestSources = station ? getLos40TestSources(station.url_resolved) : null
  const showLos40Test = isDevelopmentHost() && los40TestSources !== null
  ...
  const activeTestSource = los40TestSources && player.current?.stationuuid === station?.stationuuid
    ? player.activeSourceUrl === los40TestSources.aac ? 'aac' : 'mp3'
    : null
  const formattedClicks = station ? formatClicks(station.clickcount) : '0'
  const formattedVotes = formatClicks(stationVote.votes)
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 pb-16 pt-10 md:px-8 md:pt-16">
        {loading ? <div className="animate-pulse"><div className="h-5 w-36 rounded bg-zinc-800" /><div className="mt-8 h-56 rounded-2xl bg-zinc-900" /></div> : error || !station ? <div className="grid min-h-96 place-items-center text-center"><div><WarningCircle size={42} className="mx-auto text-zinc-600" /><h1 className="mt-5 text-2xl font-bold">No encontramos esta emisora</h1><p className="mt-2 text-zinc-500">{error}</p><a href="/" className="mt-6 inline-block rounded-xl bg-lime-300 px-5 py-3 text-sm font-bold text-zinc-950">Explorar emisoras</a></div></div> : <>
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
            <article><div className="flex flex-col gap-7 sm:flex-row sm:items-center"><StationLogo src={station.favicon} name={station.name} className="size-36 sm:size-44" /><div className="min-w-0"><p className="text-xs font-bold tracking-[.16em] text-lime-300">RADIO EN DIRECTO</p><h1 className="mt-3 text-4xl font-black leading-none tracking-[-.045em] sm:text-5xl">{station.name}</h1><p className="mt-4 text-zinc-400">{station.state || station.country || 'Emisora online'}{station.tags ? ` · ${station.tags.split(',').slice(0, 3).join(', ')}` : ''}</p><div className="mt-4 flex flex-wrap items-center gap-2">{showClicks && <span className="inline-flex items-center gap-2 rounded-full border border-lime-300/15 bg-lime-300/[.06] px-3 py-1.5 text-xs font-semibold text-lime-200"><CursorClick size={15} /> {formattedClicks} clics en las últimas 24 h</span>}{stationVote.supported && <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[.035] px-3 py-1.5 text-xs font-semibold text-zinc-300"><ThumbsUp size={14} /> {formattedVotes} votos</span>}{showClicks && clickTrend !== 0 && <span className={`inline-flex items-center gap-1 text-xs ${clickTrend > 0 ? 'text-emerald-400' : 'text-zinc-500'}`}>{clickTrend > 0 ? <TrendUp size={14} /> : <TrendDown size={14} />}{clickTrend > 0 ? '+' : ''}{clickTrend} respecto al día anterior</span>}</div><div className="mt-6 flex flex-wrap items-center gap-3"><button onClick={togglePlayback} className="flex h-12 items-center gap-2 rounded-xl bg-lime-300 px-5 font-bold text-zinc-950 transition hover:bg-lime-200 active:scale-[.98]">{isPlaying ? <Pause size={19} weight="fill" /> : <Play size={19} weight="fill" />} {isPlaying ? 'Pausar' : 'Escuchar ahora'}</button><button onClick={() => toggle(station)} aria-label="Cambiar favorito" className="grid size-12 place-items-center rounded-xl border border-white/10 text-zinc-400 transition hover:text-lime-300"><Heart size={20} weight={isFavorite(station.stationuuid) ? 'fill' : 'regular'} /></button>{stationVote.supported && <button type="button" onClick={() => void stationVote.vote()} disabled={stationVote.submitting || stationVote.votedRecently} aria-label={stationVote.votedRecently ? 'Voto registrado recientemente' : `Votar por ${station.name}`} title={stationVote.votedRecently ? 'Ya has votado por esta emisora recientemente' : 'Votar en Radio Browser'} className="flex h-12 items-center gap-2 rounded-xl border border-white/10 px-3 text-sm font-semibold text-zinc-300 transition hover:border-lime-300/30 hover:text-lime-300 active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-60"><ThumbsUp size={19} weight={stationVote.votedRecently ? 'fill' : 'regular'} /><span className="hidden min-[430px]:inline">{stationVote.submitting ? 'Votando…' : stationVote.votedRecently ? 'Votada' : 'Votar'}</span></button>}<button onClick={() => void shareStation(station, player.track)} aria-label="Compartir emisora" className="grid size-12 place-items-center rounded-xl border border-white/10 text-zinc-400 transition hover:text-lime-300"><ShareNetwork size={20} /></button></div>{stationVote.feedback && <p role="status" className="mt-3 text-sm text-zinc-400">{stationVote.feedback}</p>}{player.error && <p className="mt-4 text-sm text-red-400">{player.error}</p>}</div></div>
              {showLos40Test && los40TestSources && <section className="mt-8 rounded-2xl border border-amber-300/20 bg-amber-300/[.05] p-5"><p className="text-xs font-bold tracking-[.14em] text-amber-300">PRUEBA LOS40 · SOLO DESARROLLO</p><h2 className="mt-2 text-lg font-bold">Comparar formato de audio</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">Prueba cada formato durante varios minutos en el televisor. Cambiarlo reinicia la reproducción.</p><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={() => void player.playStation(station, los40TestSources.mp3)} aria-pressed={activeTestSource === 'mp3'} className={`rounded-xl border px-4 py-2.5 text-sm font-semibold transition ${activeTestSource === 'mp3' ? 'border-lime-300 bg-lime-300 text-zinc-950' : 'border-white/10 text-zinc-300 hover:border-lime-300/40'}`}>MP3 actual</button><button type="button" onClick={() => void player.playStation(station, los40TestSources.aac)} aria-pressed={activeTestSource === 'aac'} className={`rounded-xl border px-4 py-2.5 text-sm font-semibold transition ${activeTestSource === 'aac' ? 'border-lime-300 bg-lime-300 text-zinc-950' : 'border-white/10 text-zinc-300 hover:border-lime-300/40'}`}>AAC alternativo</button></div><p className="mt-4 text-xs text-zinc-500">Fuente alternativa: <a href={los40TestSources.attributionUrl} target="_blank" rel="noopener noreferrer" className="underline transition hover:text-zinc-300">TDTChannels</a></p></section>}
